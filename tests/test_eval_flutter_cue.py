...
"""End-to-end behavioral test for the flutter-cue takeaway mechanism.

A single-band, single-channel flutter detector (the same env -> variance +
autocorr -> fused EWS score -> adaptive-percentile inhibit -> mute chain as
examples/flutter_cue.refrain, minus the band/channel fan-out for speed) is run
over synthetic EEG. We assert the **takeaway** engages: tonic `audio_gain` (1.0)
that the flutter inhibit briefly mutes to 0.0 once the adaptive baseline is warm.

This is the integration check that the constant `reward = 1.0` is actually muted
by an inhibit (the "no operant target, only takeaway" design) — verified on both
the Python and Rust backends via the `backend` fixture.
"""
...
# Minimal flutter detector: one band (alpha), one channel, short windows so the
# adaptive baseline warms quickly. rectify("score") is an identity on the [0,1]
# score (an inhibit metric must be a primitive call).
_FLUTTER_MIN = """
    protocol "flutter_min" {
      meta { version = "1.0"; evidence = "demo"; description = "single-band flutter detector" }
      ...
      inhibit "flutter"  { metric = rectify("score"); threshold = percentile(target_pct: 80, window: 5 s); action = mute(release: 300 ms) }
...
def test_flutter_takeaway_engages(backend):
    ir = resolve(parse(_FLUTTER_MIN))
    n_samples = 40 * SR
    ...
    # The takeaway engages: the constant reward IS muted by the flutter inhibit
    # (proves mute gates a contingency-free output), at a sane self-calibrated
    # rate — neither never nor always.
    assert muted_frac > 0.02, f"takeaway never fired (muted_frac={muted_frac:.3f})"
    assert muted_frac < 0.6, f"audio muted almost constantly (muted_frac={muted_frac:.3f})"
    # And it is a genuine takeaway: muted samples drop to ~0.
