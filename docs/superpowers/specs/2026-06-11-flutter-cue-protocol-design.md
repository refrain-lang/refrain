# Dynamical critical-fluctuation cue protocol — design

> Status: approved design (brainstorm complete), ready for an implementation plan.
> Builds on `main` (v0.9.0 — the `number` control kind).
> Origin: explore whether a *dynamical*, early-warning-signal neurofeedback protocol —
> detect a critical fluctuation (a surge in EEG variability that precedes a critical
> transition) and cue the trainee with a brief audio interruption (the cue) — is
> expressible in Refrain, and what minimal language additions make it clean.
> Scope decided with the requester: build the **single-configuration** mechanism now;
> the session-arc ("phases" / macro-level adaptive flow) and infra-low bands go to the gap RFC.

## Goal

Implement, transparently and runnably, dynamical early-warning-signal neurofeedback —
not any vendor's proprietary math. Strip the source idea to its signal-processing
essence:

1. **Sense** — any site(s), referential to linked ears, ≥256 Hz (a `set` placement
   defaulting to one midline electrode; bind any montage).
2. **Decompose** — ~10 band envelopes per site.
3. **Detect a critical fluctuation** — a brief *surge in the variability* of a band
   envelope, the signature that precedes a critical transition. The literature calls
   this an *early-warning signal / critical slowing down*: rising **variance** and
   rising **lag-1 autocorrelation** near a critical transition.
4. **Cue** — on a critical fluctuation, a brief **negative-feedback cue**: the tonic
   audio is muted for a few hundred ms, then resumes. No targets, no operant reward —
   the interruption is information; the brain's orienting response does the rest.

This is a **defensible, vendor-neutral implementation**, grounded in published
...
- **Yang, Shew, Roy & Plenz (2012),** *J. Neurosci.* 32(3):1061–1072 — "Maximal
  ...
  synchrony is maximal. <https://www.jneurosci.org/content/32/3/1061>
...
- A **band fan-out**: author the per-envelope subgraph once, replicate it over a
  `bands { … }` list (× the bound sites) — extends the existing per-site fan-out.
- The example protocol `critical_fluctuation_cue.refrain`: any site(s), 10 bands per
  site, per-envelope EWS detector, cue feedback, clinician controls. Resolves and
  runs on the evaluator.

**Out of scope (→ gap RFC, `docs/proposals/`):**

- The **session phases** (warm-up → letting-go → working-deeply → release) and a
  **macro-level adaptive flow** between them. Refrain already expresses staged sessions
  (`session { phases }`, staged-protocols design) and the host can orchestrate; the
  critical-fluctuation mechanism doesn't need the arc. The *micro* level of adaptation
  (adaptive sensitivity) **is** in scope — it falls out of the adaptive threshold for free.
- **Infra-low / sub-Hz bands** (some systems claim to 0.001 Hz). Variance/autocorrelation
  there need minutes-long windows — impractical for a live cue. Capped at ~1 Hz.
- A **first-class negative-feedback cue output**. We express the cue today
  by repurposing `inhibit { … mute(release:) }` over a contingency-free `reward = 1.0`;
  the RFC proposes a real cue output so this isn't a (clean but inverted) hack.
- **AND-combine over arbitrary booleans.** We get tunable OR↔AND today by fusing
  *normalized* indicators into one score (below); a general boolean→cue path is an
...
- **Fan-out** lives in `src/refrain/fanout.py` (27 KB): given a parameterized input it
  ...
  into the `bandpass(band:)` slot) and composes with the site axis for the cross
...
Montage — site-general (a `set` placement defaulting to one midline electrode; bind any site(s)):

```refrain
controls {
  sites = placement { kind = "set"; default = ["Cz"]; allowed = "any"; min = 1; max = 8 }
}
input "raw" { montage = referential(active: sites, reference: "linked_ears") }
```

Per **band b × site s** (authored once, fanned out — see §3), the EWS detector:
...
// adaptive, self-calibrating threshold (= adaptive sensitivity) + the cue
...
Global — tonic audio, interrupted on any critical fluctuation:

```refrain
reward { continuous = 1.0 }                  // contingency-free; no operant target
output { audio_gain = reward.continuous }    // 1.0 tonically; any active critical_fluctuation inhibit mutes it
```
...
- The band axis reuses `fanout.py`'s transitive-closure replication, substituting the
  ...
  montage channel slot), naming copies `env@beta1`, `score@beta1`,
  `critical_fluctuation@beta1`, ….
- **The one genuinely new capability:** a *second* replication axis (the bound sites)
  → the cross product of `{bands} × {sites}` (e.g. 10 bands × 2 sites = 20 envelopes).
  Current `fanout.py` handles a single `set`. The implementation plan chooses the
  composition: (a) reuse the existing per-site `set` for the site axis and add a band
  axis, composing the two passes into a cross product; or (b) a generalized multi-axis
  fan-out. Foundation (single-axis transitive-closure replication, flat-AST emission) is
  unchanged; this is the design question the plan resolves.
...
10 bands per site × the bound sites (e.g. 10 bands × 2 sites = **20 envelopes**), one
detector per band per site, practical EEG range:
...
### 5. Clinician controls (the adaptive-sensitivity surface)

| control | kind | role |
|---|---|---|
| `sensitivity` | `percent` | target percentile of the EWS score; higher ⇒ fewer, more selective cues. Self-calibrating per trainee per band = **adaptive sensitivity**. |
| `hold_ms` | `number`/duration | cue length + refractory (`mute(release:)`). |
...
`reward.continuous = 1.0` ⇒ `audio_gain` is tonic 1.0. Each `critical_fluctuation@<band>`
inhibit, when its EWS score crosses the adaptive percentile, fires
`mute(release: hold_ms)`; because inhibits OR-compose on output delivery, **any**
envelope's critical fluctuation mutes the audio for `hold_ms`, then it resumes. That is
the brief informational interruption — no reward to chase, which is the entire point.
...
> behavior is guaranteed. A parity test asserts the cue fires on both backends.

## Limitations / honest caveats

- **Implementation, not a clone.** Variance + lag-1 autocorrelation is *a* defensible
  operationalization of a critical fluctuation, grounded in the EWS literature. It is
  not a vendor's proprietary, closed dynamical-systems math.
...
- **Parity:** `autocorr` Python↔Rust to machine precision via `check_equivalence.py`
  fixtures (the `coherence` parity pattern). Add `critical_fluctuation_cue` to the
  protocol corpus.
...
- **Fan-out:** assert the fanned-out AST (per band × per site) matches a hand-written
  reference, and that the cross-product naming (`env@beta1`, …) and scoping check pass.
- **Behavioral:** synthetic "calm → variance/autocorrelation surge → calm" drives a
  cue (mute) on both backends; quiet input produces tonic audio with no mutes.

## Deliverables

1. **`autocorr` primitive** + **band fan-out** (the two new language pieces).
2. **`critical_fluctuation_cue.refrain`** example protocol (resolves + runs; demo uses host-side music).
...
- **Session phases / macro-level adaptive flow** — modeling the session arc and automatic
  flow between configurations (relation to `session { phases }` + staged-protocols).
- **First-class negative-feedback cue output** — so pure-negative-feedback
  protocols don't need the constant-reward-plus-inhibit shape; plus a general
  boolean→cue path (enables AND-combine on raw conditions).
...
- **Band fan-out generalization** — multi-axis fan-out as a first-class feature (now
  prototyped here for band × site).

## References

See "Scientific grounding" above. The approach is general and vendor-neutral, grounded
in the published early-warning-signals / critical-slowing-down literature.
