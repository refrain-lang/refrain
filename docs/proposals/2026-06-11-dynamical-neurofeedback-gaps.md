...
> `autocorr` primitive, the `bands` fan-out, and `examples/flutter_cue.refrain`.
> Captures what a NeurOptimal-style protocol wants that Refrain does not yet
> express cleanly — including several gaps found while *implementing* the PoC.

## Context

`flutter_cue.refrain` reconstructs the core of NeurOptimal — detect "flutter" (an
early-warning surge in EEG variability) and cue with a brief audio "takeaway" —
using two new language pieces (`autocorr`, band fan-out) plus existing primitives.
It runs end-to-end on both backends. Building it surfaced the gaps below; none
block the PoC, and each has a documented workaround in the shipped example.

## A. Scope deferred from the design (known going in)

### A1. Session phases / macro-AutoNav

NeurOptimal flows a session through phases (warm-up → letting-go → working-deeply
→ release) and "AutoNav" navigates between them, retuning targets/sensitivity
moment to moment. The PoC implements only the **micro** level of AutoNav — the
self-calibrating `percentile` threshold per envelope. The **macro** arc (different
band/sensitivity configs over a session, with automatic transitions) is unmodeled.
Refrain already has `session { phases }` and staged/segmented protocols; a
macro-AutoNav would add **condition-driven** phase transitions and per-phase
sensitivity. Out of scope until a concrete clinical ask.
...
fast variance/CSD detectors. Document the regime boundary; infra-low flutter is a
distinct slow-detector design (long windows, DC coupling, decimation).

## B. First-class "takeaway" feedback (the biggest ergonomic gap)
...
not a reference to a pre-computed derive. The flutter detector thresholds a
computed `score` derive, so the PoC writes `metric = rectify("score")` — `rectify`
is an identity on the `[0,1]` score that satisfies the "metric is a call" rule.
**Proposal:** allow `metric = "score"` (a stream-ref) directly. The resolver
already has `_resolve_string_as_reference`; the change is small on the front end,
but the evaluator (both backends) must also accept a stream-ref metric — so it is
its own slice, not free.
...
`takeaway(on:)` output (B) closes this: `on = all_of([above(varN, tv), above(acN,
ta)])`.
...
channel cross product by composition (`<name>@<band>@<site>`). A general N-axis
...
This RFC does not propose reproducing NeurOptimal's proprietary JTFA/NDS math. The
flutter-cue work is a transparent, citable reconstruction of the *mechanism*, not
a clone, and is research software, not a medical device.
